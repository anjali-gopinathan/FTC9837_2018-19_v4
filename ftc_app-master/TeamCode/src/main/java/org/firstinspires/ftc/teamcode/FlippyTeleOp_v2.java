 ...
 */
@TeleOp(name="Flippy", group="Pushbot")
public class FlippyTeleOp_v2 extends LinearOpMode{
    Hardware9837_flippy_v2 flippy = new Hardware9837_flippy_v2();
    @Override
    public void runOpMode(){
            flippy.init(hardwareMap);
            waitForStart();
            while (opModeIsActive()) {
                ...
                //driving motors:
                flippy.rightMotor.setPower(rightDrive);
                flippy.leftMotor.setPower(leftDrive);

                //flips robot with dpad up/down and 'y' and 'a' keys
                if (flipperUp)
                    flippy.deadwheelFlipper.setPower(motorPower_flippers);
                else if (flipperDown)
                    flippy.deadwheelFlipper.setPower(-1 * motorPower_flippers);
                else
                    flippy.deadwheelFlipper.setPower(0.0);

                if (leftFlipperUp) flippy.leftFlipper.setPower(motorPower_flippers);
                else if (leftFlipperDown)
                    flippy.leftFlipper.setPower(-1 * motorPower_flippers);
                else flippy.leftFlipper.setPower(0.0);
